...
def adjust_learning_rate(optimizer, epoch, lr_list, mlt):
    decay_start_epoch = 50
    decay_end_epoch = 100
    initial_lr = utils.LEARNING_RATE * mlt
...
def train_func(disc_Y, disc_O, gen_Y, gen_O, opt_disc_Y,opt_disc_O, opt_gen, g_scaler, d_Y_scaler, d_O_scaler, L1, mse, loader, fake_young_buffer, fake_old_buffer):
    ...
    for idx, (old, young) in enumerate(loop):
        ...
        with torch.cuda.amp.autocast():
            fake_young, gen_Y_attention = gen_Y(old)
            ...
            fake_old, gen_O_attention = gen_O(young)
            ...
            D_O_loss = (D_O_real_loss + D_O_fake_loss)/2
...
def main():
    ...
    for epoch in range(start_epoch, utils.NUM_EPOCHS):
        adjust_learning_rate(opt_gen, epoch, gen_lr_list, 4)
        adjust_learning_rate(opt_disc_Y, epoch, discy_lr_list, 1)
        adjust_learning_rate(opt_disc_O, epoch, discy_lr_list, 1)
